...
for(measure in .MEASURES) {
  for(collection in .COLLECTIONS) {
    ...
    in.path <- file.path("scratch/02-transform/", paste0(collection, "_", measure))
    out1.path <- file.path("scratch/04-transform_select/", paste0(collection, "_", measure))
    out2.path <- file.path("output/04-transform_select/")
    ...
    for(i in 1:nrow(fits)) {
      ...
      if(length(eff.files) > 0) { # where transformation didn't fail
